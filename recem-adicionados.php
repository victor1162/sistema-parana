...
<html lang="pt-br">
  ...
  <div id="container" onclick="fecharNavbarSemFoco()">
    ...
    <div class="mudar-lavagem-especiais">
      <span class="btn-mudar-lavagem" onclick="abrirComumOp()">Lavagem comum</span>
      <span class="btn-mudar-lavagem" onclick="abrirEspeciaisOp()">Lavagem especiais</span>
    </div>
    <!-- lavagem COMUM -->
        ...
        <h3>Recem adicionados</h3>
          <div id="area-table-recem-adicionados">
            <table>
              ...
              <?php foreach ($recemDoDia as $key => $recemDoDia) { ?>
              <tr class="conteudo-td-recem-adicionados">
                ...
                <td>
                  <div class="btn-editar">
                    <span onclick="abrirModal('abrir', '<?= $recemDoDia['id'] ?>')"><i class="bi bi-pencil-square"></i></span>
                  </div>
                </td> 

                <td>
                  <div class="btn-exluir">
                    <span onclick="removerVeiculo('remover', <?= $recemDoDia['id'] ?>)"><i class="bi bi-trash"></i></span>
                  </div>
                </td>
              </tr>
              ...
              <tr class="rodape-tabela-recem-add">
                <td>
                  ...
                  <input type="hidden" value="<?= $recemDoDia['id'] ?>" class="value-modal">
                </td>
              </tr>

                <!-- area do modal -->
              <div class="area-modal-edita id-<?= $recemDoDia['id'] ?>">

                <div class="modal-edita">
                    <h3>Editar</h3>
                    <form method="post" action="veiculo-controller.php?acao=atualizar" class="form-edit">
                      <input type="hidden" name="id" value="<?= $recemDoDia['id'] ?>">
                      ...
                      <div class="area-salvar">
                        <div class="salvar-nao-salvar">
                          <input type="button" value="fechar" onclick="fecharModal('fechar', <?= $recemDoDia['id'] ?>)">
    ...
    <section id="area-recem-adicionados-especiais">
      ...
      <?php foreach ($recemOpcionaisDoDia as $key => $opcionaisDoDia){ ?>
      <div class="area-item-especiais">

        <i class="bi bi-trash remover-item-especial" onclick="removerEspeciais('remover', <?= $opcionaisDoDia['id'] ?>)"></i>
        ...
        <div class="area-valor-info">
          ...
          <span class="info-especiais" onclick="abrirEspeciais('abrir', <?= $opcionaisDoDia['id'] ?>)"><i class="bi bi-info-square"></i></span>
          <input type="hidden" class="valueModalOp" value="<?= $opcionaisDoDia['id'] ?>">
        </div>
      </div>
      

      <!-- modal dinamica itens especiais -->
      <div class="area-modal-especiais idOp-<?= $opcionaisDoDia['id'] ?>">
        ...
        <div class="body-modal-especiais">
            ...
            <div class="area-das-tabelas-especiais" >
              ...
              <div class="area-btn-fechar-info" onclick="fecharModalEspeciais('fechar', <?= $opcionaisDoDia['id'] ?>)">
  ...
  <script>
    function removerEspeciais(acao, id){
      window.location.href = 'recem-adicionados.php?acao=removerEspeciais&id=' + id;
    }

    let url = window.location.href;
    let encontrar = url.indexOf('id');
